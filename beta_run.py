...
def optimize_beta(model_dict, uses_tags, uses_context, split_name, dataset_name, data_dir, grid_search = False):
    ...
    this_beta_results_surp.to_csv(join(this_exp_path, 'beta_search_results.csv'))
...
if __name__ == '__main__':
    ...
    print("Because there exists two 'all' data splits, one from Dr. Meylan and one from my refactored code,")
    print("The code currently uses the split 'all_old', which is Dr. Meylan's text/the huggingface dataset with that text, for initial replications.")
    
    #which_splits = ['all_old', 'age', 'child']
    ...
    this_utts_sample = load_splits.sample_successes(this_split, this_dataset_name, data_dir_base, n = 5, regenerate = False)
