...
#SBATCH --output=/om2/user/wongn/child-directed-listening/experiments/no_versioning/scores/n=500/val/child/Naima/%j_training_beta_tags=True.out
...
singularity exec --nv -B /om,/om2/user/wongn /om2/user/wongn/vagrant/trans-pytorch-gpu python3 run_mlm.py --train_file /om2/user/wongn/child-directed-listening/finetune/child/Naima/train.txt --validation_file /om2/user/wongn/child-directed-listening/finetune/child/Naima/val.txt --cache_dir ~/.cache/$SLURM_JOB_ID --do_eval  --do_train  --eval_steps 10 --evaluation_strategy steps --learning_rate 0.0001 --load_best_model_at_end  --logging_steps 10 --logging_strategy steps --metric_for_best_model eval_loss --model_name_or_path bert-base-uncased --num_train_epochs 10 --overwrite_output_dir  --per_device_eval_batch_size 128 --per_device_train_batch_size 128 --save_steps 10 --save_strategy steps --save_total_limit 1; singularity exec --nv -B /om,/om2/user/wongn /om2/user/wongn/vagrant/trans-pytorch-gpu  python3 run_beta_search.py --split child --dataset Naima --context_width 0 --use_tags True --model_type childes
