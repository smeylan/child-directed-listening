...
singularity exec --nv -B /om,/om2/user/${CDL_SLURM_USER} ${CDL_SINGULARITY_PATH} python3 src/gen/gen_training_scripts.py & singularity exec --nv -B /om,/om2/user/${CDL_SLURM_USER} ${CDL_SINGULARITY_PATH} python3 src/gen/gen_sample_scripts.py; chmod u+x output/SLURM/submission_scripts/submit_non_child_train.sh & chmod u+x output/SLURM/submission_scripts/submit_non_child_beta_time_shelf.sh;
